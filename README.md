...
一個完整的臺股資料管理系統，提供**歷史資料獲取**、**三年新高檢查**及 **Line 即時通知**功能。
...
### 🚀 三年新高檢查
- ✅ **自動檢測**: 每天自動檢查股票是否創三年新高
- ✅ **Line 通知**: 發現新高立即推送 Line 訊息
- ✅ **詳細資訊**: 顯示股票代號、名稱、新高價、前高價與日期
- ✅ **定時執行**: 每天收盤後自動執行（下午 3:30）

### 🔍 資料完整性
- ✅ **完整性檢查**: 自動分析缺失的資料並產生詳細報告
...
```env
# FinMind API Token（可選，但強烈建議）
FINMIND_API_TOKEN=your_token_here

# Line Messaging API（用於新高通知）
...
**功能**: 檢查最新日期的股價是否為近三年新高，並透過 Line 通知
...
在 Linux 伺服器上設定自動定時任務，自動獲取股票資料並檢查新高。
...
- **執行頻率**: 每小時一次
- **日誌檔案**: `/var/log/stock-fetcher.log`
- **功能**: 自動獲取並更新股票資料
...
- **執行頻率**: 每天下午 3:30（收盤後）
- **日誌檔案**: `/var/log/check-new-high.log`
- **功能**: 檢查新高並發送 Line 通知
...
#### 三年新高通知
當有股票創新高時，會收到類似以下的 Line 訊息：

```
🚀 創 3 年新高通知
📅 2026-01-05 (共 11 支)

53 (元大電子): 新高 $150.15 | 前高 $144.20 (2025-12-30)
2330 (台積電): 新高 $1695.00 | 前高 $1585.00 (2024-07-11)
2337 (旺宏): 新高 $47.65 | 前高 $43.35 (2025-04-21)
...
```
...
│   ├── manage.sh                # 服務管理腳本
│   ├── stock-fetcher.service    # 股票資料獲取 service
│   ├── stock-fetcher.timer      # 股票資料獲取 timer
│   ├── check-new-high.service   # 新高檢查 service
│   └── check-new-high.timer     # 新高檢查 timer
├── data/                        # 資料目錄（自動產生）
...
- ✨ **新增三年新高檢查功能** (`check_new_high.py`)
  - 自動檢測股票是否創三年新高
  - Line 即時通知推送
  - 顯示股票代號、名稱、新高價、前高價與日期
- ✨ **自動化部署升級**
  - 新增 `check-new-high.service` 和 `check-new-high.timer`
  - 每天下午 3:30 自動檢查新高
  - 安裝腳本自動建立虛擬環境
- ✨ **Line 通知整合**
  - 股票資料獲取完成通知
